...
$mysqli = mysqli_connect('localhost', 'u256704804_ta', 'HU8RwS6TBULZ', 'u256704804_ta');
    ...
    if ($api == "nasabah") {
        ...
        if($jml>0){
            ...
            //menguranig saldo tabungan umum sebagai biaya transfer dikenakan kepada pengirim
                $query3 = mysqli_query($mysqli,"INSERT INTO tbltabungan(jenis_transaksi,id_akun,kode_transaksi,no_rekening,no_rekening_pengirim,jumlah_debit,tanggal_simpan) VALUES ('BIAYA_TRANFER','2101','$kode_transaksi2','$no_rekening_pengirim','ADM-TF','2000','$tanggal_simpan')");
        ...
        if($jml>0){
            ...
            $data_anggota = array(
                "kode"   => 404,
                "status" => "Tidak ada",
                "sisa_kredit" => "Rp ".number_format (0 , 2, ',', '.')
        ...
        $TRANS_MASUK    = mysqli_query($mysqli,"SELECT * FROM tbltabungan INNER JOIN tblnasabah ON tbltabungan.no_rekening_pengirim = tblnasabah.no_rekening WHERE tbltabungan.jenis_transaksi = 'TRANS_MASUK' AND tbltabungan.no_rekening = '$no_rekening' ORDER BY tbltabungan.id_tabungan DESC LIMIT 5");
        $TRANS_KELUAR   = mysqli_query($mysqli,"SELECT * FROM tbltabungan INNER JOIN tblnasabah ON tbltabungan.no_rekening_pengirim = tblnasabah.no_rekening WHERE tbltabungan.jenis_transaksi = 'TRANS_KELUAR' AND tbltabungan.no_rekening = '$no_rekening' ORDER BY tbltabungan.id_tabungan DESC LIMIT 5");
        $simpan         = mysqli_query($mysqli,"SELECT * FROM tbltabungan WHERE jenis_transaksi = 'simpan' AND no_rekening = '$no_rekening' ORDER BY id_tabungan DESC LIMIT 5");
        $TARIK_TUNAI    = mysqli_query($mysqli,"SELECT * FROM tbltabungan WHERE jenis_transaksi = 'TARIK_TUNAI' AND no_rekening = '$no_rekening' ORDER BY id_tabungan DESC LIMIT 5");
      ...
      if (!empty($data)) {
          ...
          $query1 = mysqli_query($mysqli,"UPDATE tblnasabah SET $field = '$data' WHERE no_rekening = '$no_rekening'");
          $data_update = array(
            "kode"   => 201,
            "status" => $nama ."berhasil di update",
            );
        ...
        $arr = array(
            "kode" => 401,
            "status" => "Unauthorized",
            "Info" => "Api Tugas Akhir | Aplikasi Transaksi Nasabah Bank Mini IQTI Berbasis Android,by Dhiyan Iryadi",
        );
        echo json_encode($arr);
    }
}
